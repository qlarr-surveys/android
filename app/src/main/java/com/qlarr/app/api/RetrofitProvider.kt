...
object RetrofitProvider {
    ...
    fun retrofitPublicEndpoints(sessionManager: SessionManager): Retrofit =
        getRetrofit(
            DUMMY_URL, getHttpClientBuilder(60)
    ...
    fun retrofitGuest(): Retrofit =
        getRetrofit(BuildConfig.CLOUD_SERVER_URL, getHttpClientBuilder(60).build())
    ...
    fun retrofitAuthenticatedEndpoints(
        ...
        val httpClient = getHttpClientBuilder(callTimeout)
            .addInterceptor(BaseUrlInterceptor {
                sessionManager.env()?.baseUrl
            })
...
class BaseUrlInterceptor(var urlConfigHolder: () -> String?) : Interceptor {
